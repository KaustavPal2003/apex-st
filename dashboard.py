...
show_trend = st.sidebar.checkbox("Show trend & regime panel", value=True)

st.sidebar.divider()
st.sidebar.caption("**Trend & Regime Analysis**")
st.sidebar.caption("Run to (re)generate:")
st.sidebar.code("python trend_analysis.py", language="bash")
...
@st.cache_data(ttl=120)
def load_slope_correlation() -> pd.DataFrame:
    """Load results/slope_correlation.csv written by trend_analysis.py."""
    p = Path("results/slope_correlation.csv")
    return pd.read_csv(p) if p.exists() else pd.DataFrame()


@st.cache_data(ttl=120)
def load_regime_classification() -> pd.DataFrame:
    """Load results/regime_classification.csv written by trend_analysis.py."""
    p = Path("results/regime_classification.csv")
    return pd.read_csv(p) if p.exists() else pd.DataFrame()


@st.cache_data(ttl=120)
def load_trend_prediction_accuracy() -> pd.DataFrame:
    """Load results/trend_prediction_accuracy.csv written by trend_analysis.py."""
    p = Path("results/trend_prediction_accuracy.csv")
    return pd.read_csv(p) if p.exists() else pd.DataFrame()
...
# ─────────────────────────────────────────────────────────────────────────────
# SECTION 1B: TREND & REGIME ANALYSIS  (Sprint 7 — trend_analysis.py)
# ─────────────────────────────────────────────────────────────────────────────
if show_trend:
    st.subheader("📐 Trend & Regime Analysis")
    st.caption(
        "Rolling 20-day slope correlation (predicted vs actual trend direction) "
        "and HMM regime classification. Generated by `trend_analysis.py`."
    )

    slope_df  = load_slope_correlation()
    regime_df = load_regime_classification()
    tpa_df    = load_trend_prediction_accuracy()

    if slope_df.empty and regime_df.empty and tpa_df.empty:
        st.info(
            "No trend/regime artefacts found. Run:\n\n"
        ...
        # ── Slope correlation ───────────────────────────────────────────
        with tc1:
            st.markdown("**Slope Correlation — Predicted vs Actual Trend**")
            if not slope_df.empty:
                sdisp = slope_df.copy()
                sdisp = sdisp.rename(columns={
                    "symbol":               "Symbol",
                    "slope_correlation":    "Slope Corr",
                    "slope_corr_pvalue":    "p-val",
                    "direction_agree_pct":  "Dir Agree %",
                    "current_actual_trend": "Actual",
                    "current_pred_trend":   "Pred",
                    "trend_match":          "Match",
                })
                cols = ["Symbol", "Slope Corr", "p-val", "Dir Agree %",
                        "Actual", "Pred", "Match"]
                cols = [c for c in cols if c in sdisp.columns]
                st.dataframe(
                    sdisp[cols].set_index("Symbol"),
                    use_container_width=True,
                )

                avg_corr  = slope_df["slope_correlation"].mean()
                avg_agree = slope_df["direction_agree_pct"].mean()
                n_match   = (slope_df["trend_match"] == "✅").sum()
                mc1, mc2, mc3 = st.columns(3)
                mc1.metric("Mean slope corr", f"{avg_corr:+.3f}")
                mc2.metric("Mean dir agree", f"{avg_agree:.1f}%")
                mc3.metric("Current trend match", f"{n_match}/{len(slope_df)}")
            else:
                st.caption("No slope correlation data — check ensemble_result.json artefacts.")

        # ── Regime classification ───────────────────────────────────────
        with tc2:
            st.markdown("**Current Regime — per Symbol**")
            if not regime_df.empty:
                rdisp = regime_df.rename(columns={
                    "symbol":        "Symbol",
                    "current_label": "Regime",
                    "current_state": "State",
                    "mean_ret":      "Mean Ret",
                    "std_ret":       "Std Ret",
                    "freq_pct":      "Freq %",
                })
                cols = ["Symbol", "Regime", "State", "Mean Ret", "Std Ret", "Freq %"]
                cols = [c for c in cols if c in rdisp.columns]
                st.dataframe(
                    rdisp[cols].set_index("Symbol"),
                    use_container_width=True,
                )

                from collections import Counter
                counts = Counter(regime_df["current_label"])
                st.caption("Regime distribution across watchlist:")
                for label, cnt in counts.most_common():
                    st.write(f"{label} — {'█' * cnt} ({cnt})")
            else:
                st.caption("No regime data — check *_apex_pipeline.pkl / data/<SYM>.csv artefacts.")

        # ── Trend prediction accuracy ─────────────────────────────────
        with tc3:
            st.markdown("**Trend Prediction Accuracy**")
            if not tpa_df.empty:
                tdisp = tpa_df.rename(columns={
                    "symbol":                    "Symbol",
                    "evaluated_days":             "Days",
                    "trend_prediction_accuracy":  "Accuracy %",
                })
                cols = ["Symbol", "Days", "Accuracy %"]
                cols = [c for c in cols if c in tdisp.columns]
                st.dataframe(
                    tdisp[cols].set_index("Symbol"),
                    use_container_width=True,
                )

                avg_acc = tpa_df["trend_prediction_accuracy"].mean()
                best    = tpa_df.loc[tpa_df["trend_prediction_accuracy"].idxmax()]
                worst   = tpa_df.loc[tpa_df["trend_prediction_accuracy"].idxmin()]
                st.metric("Mean trend prediction accuracy", f"{avg_acc:.1f}%")
                st.caption(
                    f"Best: **{best['symbol']}** ({best['trend_prediction_accuracy']:.1f}%)  ·  "
                    f"Worst: **{worst['symbol']}** ({worst['trend_prediction_accuracy']:.1f}%)"
                )
                if "evaluation_timestamp" in tpa_df.columns and not tpa_df.empty:
                    st.caption(f"Last evaluated: {tpa_df['evaluation_timestamp'].max()}")
            else:
                st.caption("No trend prediction accuracy data — check ensemble_result.json artefacts.")
...
if sym_choice:
    ...
    with m_cols[4]:
        ...
        trend_delta = ""
        if not regime_df.empty:
            rrow = regime_df[regime_df["symbol"] == sym_choice]
            if not rrow.empty:
                regime_label = rrow.iloc[0]["current_label"]
        if not slope_df.empty:
            srow = slope_df[slope_df["symbol"] == sym_choice]
            if not srow.empty:
                trend_delta = f"trend match: {srow.iloc[0]['trend_match']}"
        st.metric("Current Regime", regime_label, trend_delta)
        if not tpa_df.empty:
            trow = tpa_df[tpa_df["symbol"] == sym_choice]
            if not trow.empty:
                st.caption(
                    f"Trend pred. accuracy: "
                    f"**{trow.iloc[0]['trend_prediction_accuracy']:.1f}%** "
                    f"({int(trow.iloc[0]['evaluated_days'])} days)"
                )
